...
<%
	...
	notice = (NoticeDTO) session.getAttribute("notice");
%>
...
<body>
   <header>
        ...
        <main>
			...
			<form action="getNotice.no">
			<input type="hidden" name="noticeID" value="<%=notice.getNoticeID()%>">
            <table class="get_board">
                <thead>
					<tr>
						<th scope="col"><%= notice.getNoticeTitle()%></th>
						<th scope="col" width="10%">조회수 : <%= notice.getNoticeCnt()%></th>
						<th scope="col" width="20%"><%= notice.getNoticeDate()%></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td class="notice_area" colspan="3"><%= notice.getNoticeContent()%></td>
					</tr>
                    <tr>
                        <td colspan="3" style="border-bottom: none;">
                            <%
               				 ...
               				 %>
                				<input type="button" value="수정" class="btn_skyblue" onclick='location.href="notice_update.jsp?noticeID=<%= notice.getNoticeID()%>"'>
                				<input type="button" value="삭제" class="btn_navy" onclick='location.href="deleteNotice.no?noticeID=<%= notice.getNoticeID()%>"'>
                				<input type="button" value="목록" class="btn_navy" onclick='location.href="getNoticeList.no"'>
                			<%
                				} else {
               				%> 
               				<input type="button" value="목록" class="btn_navy" onclick='location.href="getNoticeList.no"'>
