...
<%
	...
	try {
	qnaList = (List<QnaDTO>) session.getAttribute("qnaList");
%>
<%
...
<body>
    <header>
        ...
        <main>
            ...
            <div class="notice_list_title"><h1>질 문 게 시 판</h1></div>
            <table class="notice_list">
				...
				<tbody>
				<%
				for (QnaDTO k : qnaList) {
				%>
					<tr>
						<td><%=k.getQnaID()%></td>
						<td><a href="getQna.qa?qnaID=<%=k.getQnaID()%>"><%= k.getQnaTitle()%></a></td>
						<td><%=k.getNickName() %></td>
						<td><%=k.getQnaCnt() %></td>
						<td><%=k.getQnaDate() %></td>
					</tr>
            ...
            <div class="page">
				<%
                	...
                	if (userID != null) {
                %>
                	<a href="qna_insert.jsp"><button class="btn_navy">글쓰기</button></a>
                <%
                	} else {
                %>
				<%
